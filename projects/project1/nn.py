...
class Layer:
    ...
    def evaluate(self, input_vec):
        ...
        return self.transfer_func(
            np.matmul(input_vec, self.weight_matrix.T) + self.bias
        )
...
class NN:
    ...
    def __init__(self, shape, weight_vector=None):
        ...
        if weight_vector is None:
            weight_cnt = sum(
                shape[i - 1] * shape[i] + shape[i] for i in range(1, len(shape))
            )
    ...
    def set_weight_vector(self, weight_vector):
        ...
        for i in range(1, len(self.shape)):
            ...
            w_mat = np.reshape(
                weight_vector[idx : idx + in_cnt * out_cnt], (out_cnt, in_cnt)
            )
            idx += in_cnt * out_cnt
            ...
            if i != len(self.shape) - 1:
                ...
                self.layers.append(
                    Layer(w_mat, bias_vec, lambda x: 1 / (1 + np.exp(-x)))
                )
