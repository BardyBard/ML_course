...
if model_to_run == "ID3":
    ...
    x_test = pca_transform(x_test, x_mean,
                           top_components)  # reapply the same to test data
    ...
    x_test_disc = apply_bins(x_test, bins).astype(
        str)  # reapply the same to test data
    ...
    final_header, final_train_data = ID3_format(tx_disc,
                                                y_train)  # format the data accordingly
    best_model = ID3()
    best_model.fit(final_header, final_train_data, best_depth)

    # Generate predictions on it
    x_test_disc = apply_bins(x_test, bins)
    dummy_column_test = np.full((x_test_disc.shape[0], 1),
                                "x")  # another placeholder
    test_header, test_data = ID3_format(x_test_disc,
                                        dummy_column_test)  # format the data accordingly
    ...
    drop_columns = ["FRUITJU1", "_AIDTST3", "HIVTST6", "_FRUTSUM", "FRUTDA1_",
                ...
                "NUMMEN", "NUMWOMEN", "CTELNUM1", "CELLFON2", "CADULT",
                "PVTRESD2", "CCLGHOUS", "CSTATE", "LANDLINE", "HHADULT",
                "POORHLTH", "BPMEDS", "ASTHNOW", "DIABAGE2", "NUMHHOL2",
                "NUMPHON2", "CPDEMO1", "PREGNANT", "SMOKDAY2", "STOPSMK2",
                "LASTSMK2", "AVEDRNK2", "DRNK3GE5", "MAXDRNKS", "EXRACT11",
                "EXEROFT1", "EXERHMM1", "EXRACT21", "EXEROFT2", "EXERHMM2",
                "LMTJOIN3", "ARTHDIS2", "ARTHSOCL", "JOINPAIN", "FLSHTMY2",
                "IMFVPLAC", "HIVTSTD3", "WHRTST10", "PDIABTST", "PREDIAB1",
                "INSULIN", "BLDSUGAR", "FEETCHK2", "DOCTDIAB", "CHKHEMO3",
                "FEETCHK", "EYEEXAM", "DIABEYE", "DIABEDU", "CAREGIV1",
                ...
                "SHINGLE2", "HADMAM", "HOWLONG", "HADPAP2", "LASTPAP2",
                ...
                "ADENERGY", "ADEAT1", "ADFAIL", "ADTHINK", "ADMOVE", "MISTMNT",
                "ADANXEV", "MSCODE", "_CRACE1", "_CPRACE", "_CLLCPWT",
                "_DUALCOR", "METVL11_", "METVL21_", "ACTIN11_", "ACTIN21_",
                "PADUR1_", "PADUR2_", "PAFREQ1_", "PAFREQ2_", "_MINAC11",
                "_MINAC21", "PAMIN11_", "PAMIN21_", "PA1MIN_", "PAVIG11_",
                "PAVIG21_", "PA1VIGM_", "_FLSHOT6", "_PNEUMO2"]
    x_train, x_test, y_train, train_ids, test_ids = load_csv_data(
    ...
    y_train_split_balanced = np.where(y_train_split_balanced == -1, 0,
                                      y_train_split_balanced)
    predictions = knn_predict_streaming(tx_train_split_balanced,
                                        y_train_split_balanced, x_test_reduced,
                                        17)
