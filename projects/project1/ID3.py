...
def ID3_format(x, y):
    ...
    train_data = np.hstack(
        (x, y)
    )  # paste x and y together to comply to ID3 method's format
...
class Node:
    ...
    def branches(self, level=1, line=None):
        ...
        if line is None:
            line = (
                []
            )  # setting line = [] in function args doesn't work because arg is a pointer to the arr
        bs = []
...
class ID3:
    ...
    def fit(
        self, _header: List[str], _dataset: List[List[str]], depth=None, verbose=False
    ):
        """
        ...
        self.tree = id3(
            self.dataset, self.dataset, self.header[:-1], self.classes, depth, verbose
        )  # root of ID3 tree

    def predict(
        self, header: List[str], dataset: List[List[str]], verbose=False
    ) -> List[str]:
        """
...
def test_hyperparams(
    x_train, y_train, max_depth: int, k=3, seed=42, subsample_size=3000
):
    """
